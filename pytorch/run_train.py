...
if __name__ == "__main__":
    ...
    print(model)
    model_fname = fd+'dp'+dp+'/unet-model'
    loss_fname = fd+'dp'+dp+'/training_valid_loss.dat'
    ...
    if n_epoch>0:
        ...
        lr = float(lr)
        train(model,X_train,y_train,mask_train,X_valid,y_valid,mask_valid,model_fname,loss_fname,lr,n_batch,n_epoch)
    else:
        with torch.no_grad():
            model.load_state_dict(torch.load(model_fname))
            ...
            fout = open(fd+'dp'+dp+'/rl_valid_pred.dat','w')
